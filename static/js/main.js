...
// Pre-fetch CSRF token on page load so it's ready for the first API call.
// This also handles the case where session.clear() on login invalidated
// the old token — the fresh fetch gets the new one.
...
function confirmSuggestMonth(month, year) {
    ...
    .then(d => {
        ...
        else alert(d.error || 'Σφάλμα δημιουργίας μήνα');
    })
