...
def get_analytics_data(user_id: int) -> dict:
    ...
    # Top expense category per month (for the analytics table "Κορυφαίο" column)
    cursor.execute(
        """SELECT month_id, category, SUM(amount) AS total
           FROM transactions
           WHERE month_id IN (SELECT id FROM months WHERE user_id=?)
             AND type='expense'
    ...
    for row in cursor.fetchall():
        mid = row['month_id']
        if mid not in top_cat_by_month or row['total'] > top_cat_by_month[mid][1]:
            top_cat_by_month[mid] = (row['category'], row['total'])
    ...
    for month in all_months:
        ...
        top = top_cat_by_month.get(month['id'])
        ms['top_expense_category'] = top[0] if top else None
        ms['top_expense_amount']   = round(top[1], 2) if top else 0
