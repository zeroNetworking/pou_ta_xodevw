...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(_data_dir, 'finance_app.log'), encoding='utf-8'),
...
def require_json_fields(data: dict, *fields):
    for field in fields:
        if data.get(field) is None:
            ...
            return False, (jsonify({'success': False, 'error': f'Λείπει το πεδίο: {field}'}), 400)
    return True, None


def service_response(result: dict):
    status = result.get('status', 200 if result.get('success') else 400)
    # Build response without the internal 'status' key
...
def new_month():
    if not validate_csrf_json():
        return jsonify({'success': False, 'error': 'Σφάλμα ασφαλείας (CSRF). Ανανέωσε τη σελίδα.'}), 403
    user_id = get_current_user_id()
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'Μη έγκυρο έτος ή μήνας'}), 400
...
def add_transaction():
    ...
    if is_rate_limited('add_transaction', client_ip):
        return jsonify({'success': False, 'error': 'Πάρα πολλά αιτήματα. Δοκίμασε αργότερα.'}), 429
...
def export_year_csv():
    ...
    try:
        ...
        return jsonify({'error': 'Μη έγκυρα IDs μηνών'}), 400
...
if __name__ == '__main__':
    init_db()
    logger.info("Που τα ξοδεύω starting on port 5001 (debug=%s)", DEBUG_MODE)
