...
from services.db        import get_db, init_db, wipe_user_data
from services.constants import (
    GREEK_MONTHS, INCOME_CATEGORIES, EXPENSE_TREE, CAT_COLORS,
    MONTH_CLOSE_THROTTLE_SECONDS,
)
from services.validation  import (
    is_valid_password, is_valid_username, safe_float, ALLOWED_TYPES,
    is_valid_recovery_answer, normalize_recovery_answer,
)
...
# ──────────────────────────────────────────────────────────────────────────────
# Password Recovery — security question helpers
# ──────────────────────────────────────────────────────────────────────────────

# Predefined questions. Limiting choice prevents users from typing PII as the
# question itself (e.g. "What's my IBAN?") and keeps the dropdown short.
RECOVERY_QUESTIONS = [
    'Πώς λεγόταν το πρώτο σου κατοικίδιο;',
    'Σε ποια πόλη γεννήθηκες;',
    'Ποιο είναι το πατρικό όνομα της μητέρας σου;',
    'Πώς λεγόταν το δημοτικό σχολείο που πήγες;',
    'Ποιο είναι το αγαπημένο σου βιβλίο;',
    'Ποιος ήταν ο πρώτος σου δάσκαλος;',
]


def hash_recovery_answer(answer: str, salt: str) -> str:
    """
    Hash the recovery answer with the SAME PBKDF2 setup as passwords.
    Answer is normalized first (lowercase + strip) so casing/spacing doesn't
    block legitimate users.
    """
    canonical = normalize_recovery_answer(answer)
    return hash_password(canonical, salt)


def verify_recovery_answer(answer: str, salt: str, stored_hash: str) -> bool:
    """Constant-time comparison, like verify_password."""
    computed = hash_recovery_answer(answer, salt)
    return hmac.compare_digest(computed, stored_hash)
...
def register():
    ...
    errors = []
    # Preserve form values across POST roundtrips so the user doesn't
    # have to retype everything on a validation failure.
    form_data = {'username': '', 'recovery_question': RECOVERY_QUESTIONS[0]}
    ...
    if request.method == 'POST':
        ...
        recovery_question = request.form.get('recovery_question', '').strip()
        recovery_answer   = request.form.get('recovery_answer', '')
        client_ip         = request.remote_addr or '0.0.0.0'

        form_data = {
            'username': username,
            'recovery_question': recovery_question or RECOVERY_QUESTIONS[0],
        }
        ...
        if recovery_question not in RECOVERY_QUESTIONS:
            errors.append('Διάλεξε μία από τις ερωτήσεις ανάκτησης.')
        if not is_valid_recovery_answer(recovery_answer):
            errors.append('Απάντηση ανάκτησης: 3–100 χαρακτήρες.')

        if not errors:
            # Two separate salts: one for the password, one for the recovery
            # answer. Sharing salts across credentials is a small leak but a leak.
            pw_salt    = secrets.token_hex(16)
            pw_hash    = hash_password(password, pw_salt)
            recv_salt  = secrets.token_hex(16)
            recv_hash  = hash_recovery_answer(recovery_answer, recv_salt)
            db         = get_db()
            cursor     = db.cursor()
            try:
                cursor.execute(
                    """INSERT INTO users
                         (username, password_hash, salt,
                          recovery_question, recovery_answer_hash, recovery_answer_salt)
                       VALUES (?,?,?,?,?,?)""",
                    (username, pw_hash, pw_salt,
                     recovery_question, recv_hash, recv_salt)
                )
                db.commit()
...
#
# 3-step flow:
#   1. /forgot          (GET)  → username form
#   2. /forgot          (POST) → show user's question, accept the answer.
#                                On correct answer, sets a short-lived token
#                                in the session that grants access to step 3.
#   3. /forgot/reset    (GET/POST) → set a new password. Token consumed on use.
#
# Why a session token instead of a URL token?
#   - The whole flow happens in one browser session; no email link involved.
#   - Storing it server-side (in session) keeps it out of URLs and browser history.

RESET_TOKEN_TTL_SECONDS = 5 * 60  # 5 minutes — long enough to type a password


def _issue_reset_token(user_id: int) -> None:
    """Mark this session as authorized to reset the given user's password."""
    session['pwd_reset'] = {
        'user_id':   user_id,
...
def _consume_reset_token() -> int | None:
    """
    Return the user_id allowed to reset their password, or None if no
    valid token exists. Token is single-use: cleared from session immediately.
    """
    token = session.get('pwd_reset')
    if not token:
        return None

    age = datetime.now().timestamp() - token.get('issued_at', 0)
...
def _peek_reset_token() -> int | None:
    """Like _consume_reset_token but does NOT clear the token (for GET requests)."""
    token = session.get('pwd_reset')
    if not token:
        return None
    age = datetime.now().timestamp() - token.get('issued_at', 0)
...
def forgot_password():
    """
    Step 1 (GET / POST without question_step): user enters username.
    Step 2 (POST with question_step=1): user answers their security question.
    """
    if get_current_user_id():
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        if not validate_csrf_form():
            return render_template('forgot.html', error='Σφάλμα ασφαλείας. Ανανέωσε τη σελίδα.')

        client_ip = request.remote_addr or '0.0.0.0'
        if is_rate_limited('forgot', client_ip):
            logger.warning("Forgot-password rate limit exceeded for IP %s", client_ip)
            return render_template(
                'forgot.html',
                error='Πάρα πολλές προσπάθειες. Δοκίμασε σε 5 λεπτά.',
            )

        question_step    = request.form.get('question_step') == '1'
        username_attempt = request.form.get('username', '').strip()

        cursor = get_db().cursor()
        cursor.execute(
            """SELECT id, username, recovery_question,
                      recovery_answer_hash, recovery_answer_salt
                 FROM users WHERE username=?""",
            (username_attempt,)
        )
        user = cursor.fetchone()

        # Important: we always render the question screen if the user *exists*
        # AND has a recovery question set. Anything else gets a generic error
        # to avoid leaking which usernames exist.
        user_has_recovery = (
            user is not None
            and user['recovery_question']
            and user['recovery_answer_hash']
            and user['recovery_answer_salt']
        )

        if not question_step:
            # Step 1 → step 2: show the question
            if user_has_recovery:
                show_question = True
                question_text = user['recovery_question']
            else:
                record_attempt('forgot', client_ip)
                error = (
                    'Δεν βρέθηκε χρήστης με αυτό το username, '
                    'ή δεν έχει ορίσει ερώτηση ανάκτησης.'
                )
        else:
            ...
            if answer_correct:
                _issue_reset_token(user['id'])
                logger.info(
                    "Password recovery answer correct for user=%s ip=%s",
                    username_attempt, client_ip
                )
                return redirect(url_for('forgot_reset'))

            record_attempt('forgot', client_ip)
            logger.warning(
                "Password recovery answer FAILED for user=%s ip=%s",
                username_attempt, client_ip
            )
            error = 'Λάθος απάντηση.'
            # Re-show the question so they can try again (rate limit will catch abuse)
            if user_has_recovery:
                show_question = True
                question_text = user['recovery_question']
...
def forgot_reset():
    """Step 3: enter a new password. Requires a valid reset token in session."""
    if get_current_user_id():
        return redirect(url_for('index'))

    user_id = _peek_reset_token()
    if not user_id:
    ...
    if request.method == 'POST':
        if not validate_csrf_form():
            return render_template('forgot_reset.html', error='Σφάλμα ασφαλείας. Ανανέωσε τη σελίδα.')

        # Re-validate the token at submission time and consume it.
        user_id = _consume_reset_token()
        if not user_id:
            return redirect(url_for('forgot_password'))
        ...
        if not is_valid_password(new_password):
            ...
            db.commit()
            logger.info("Password reset via recovery for user_id=%s", user_id)
            session.clear()
            return redirect(url_for('login'))

        # On error, re-issue the token so the user can retry without restarting
        _issue_reset_token(user_id)
...
def profile():
    ...
    if request.method == 'POST':
        ...
        # ── Set/Change Recovery Question ─────────────────────
        # Works both for first-time setup (legacy users with no recovery)
        # AND for changing an existing one. Same handler — same validation.
        elif action == 'set_recovery':
            confirm_password  = request.form.get('confirm_password', '')
            recovery_question = request.form.get('recovery_question', '').strip()
            recovery_answer   = request.form.get('recovery_answer', '')
            ...
            if not user or not verify_password(
                confirm_password, user['salt'], user['password_hash']
            ):
                error = 'Λάθος κωδικός.'
            elif recovery_question not in RECOVERY_QUESTIONS:
                error = 'Διάλεξε μία από τις ερωτήσεις ανάκτησης.'
            elif not is_valid_recovery_answer(recovery_answer):
                error = 'Απάντηση ανάκτησης: 3–100 χαρακτήρες.'
            else:
                recv_salt = secrets.token_hex(16)
                recv_hash = hash_recovery_answer(recovery_answer, recv_salt)
                cursor.execute(
                    """UPDATE users
                        ...
                        WHERE id = ?""",
                    (recovery_question, recv_hash, recv_salt, user_id)
                )
                db.commit()
                # Different message depending on first-time vs update,
                # so the user knows what just happened.
                had_recovery_before = bool(user['recovery_question'])
                success = (
                    'Η ερώτηση ανάκτησης ενημερώθηκε.'
                    if had_recovery_before
                    else 'Η ερώτηση ανάκτησης ορίστηκε.'
                )
                logger.info("User %s updated recovery question", user_id)
        ...
        # ── Delete Account ───────────────────────────────────
            ...
            if not user or not verify_password(delete_password, user['salt'], user['password_hash']):
                ...
                # All deletion order/FK logic lives in services.db.wipe_user_data
                # so it stays correct in one place.
                wipe_user_data(db, user_id)
                db.commit()
...
def _render_profile(user_id: int, success, error):
    ...
    cursor.execute(
        "SELECT username, created_at, recovery_question FROM users WHERE id=?",
        (user_id,)
    )
    user = cursor.fetchone()
    ...
    return render_template('profile.html',
        ...
        # Recovery question state — empty string if user never set one
        current_recovery_question=user['recovery_question'] or '',
        recovery_questions=RECOVERY_QUESTIONS,
        success=success,
        error=error,
...
# ==============================================================================
# Database Initialization (runs for both dev server and Gunicorn)
# ==============================================================================
init_db()

# ==============================================================================
# Entry Point (dev server only)
# ==============================================================================
