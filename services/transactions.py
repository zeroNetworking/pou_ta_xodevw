...
def add_transaction(user_id: int, data: dict) -> dict:
    ...
    if trans_type not in ALLOWED_TYPES:
        return {'success': False, 'error': f'Μη έγκυρος τύπος: {trans_type}', 'status': 400}

    # Validate amount
    amount = safe_float(data.get('amount', 0))
    if amount <= 0:
        return {'success': False, 'error': 'Το ποσό πρέπει να είναι μεγαλύτερο από 0', 'status': 400}

    if not category:
        return {'success': False, 'error': 'Η κατηγορία είναι υποχρεωτική', 'status': 400}
    ...
    if not month:
        return {'success': False, 'error': 'Ο μήνας δεν βρέθηκε', 'status': 404}
...
def edit_transaction(user_id: int, transaction_id: int, data: dict) -> dict:
    ...
    cursor.execute(
        """SELECT t.id, m.is_closed FROM transactions t JOIN months m ON t.month_id = m.id
    ...
    if not row:
        ...
        return {'success': False, 'error': 'Δεν βρέθηκε', 'status': 403}

    # Prevent editing transactions in closed months
    if row['is_closed']:
        return {'success': False, 'error': 'Δεν μπορείς να επεξεργαστείς κίνηση σε κλειστό μήνα', 'status': 403}
...
def delete_transaction(user_id: int, transaction_id: int) -> dict:
    ...
    # Prevent deleting transactions from closed months
    if row['is_closed']:
        return {'success': False, 'error': 'Δεν μπορείς να διαγράψεις κίνηση από κλειστό μήνα', 'status': 403}
